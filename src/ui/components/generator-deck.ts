...
export class GeneratorDeck {
  ...
  /**
   * Create actions panel (G077-G078)
   */
  private createActionsPanel(): HTMLElement {
    const panel = document.createElement('div');
    panel.className = 'generator-deck__actions';
    ...
    actionsRow.className = 'generator-deck__actions-row';
    
    // Freeze button (G077)
    const freezeBtn = document.createElement('button');
    freezeBtn.className = 'generator-deck__action-btn';
    freezeBtn.textContent = '❄️ Freeze';
    freezeBtn.title = 'Mark events as user-owned (prevents regeneration)';
    freezeBtn.onclick = () => this.freeze();
    actionsRow.appendChild(freezeBtn);
    
    // Humanize button (G078)
    const humanizeBtn = document.createElement('button');
    humanizeBtn.className = 'generator-deck__action-btn';
    humanizeBtn.textContent = '🎭 Humanize';
    humanizeBtn.title = 'Add natural timing and velocity variation';
    humanizeBtn.onclick = () => this.humanize(0.2);
    actionsRow.appendChild(humanizeBtn);
    
    // Quantize button (G078)
    const quantizeBtn = document.createElement('button');
    quantizeBtn.className = 'generator-deck__action-btn';
    quantizeBtn.textContent = '📏 Quantize';
    quantizeBtn.title = 'Snap events to grid (8th notes)';
    quantizeBtn.onclick = () => this.quantize(120);
    actionsRow.appendChild(quantizeBtn);
  ...
  /**
   * Freeze action (G077: mark events as user-owned)
   * Prevents future regeneration of these events
   */
  freeze(): void {
    if (!this.activeSlot) return;
    
    const eventStore = getSharedEventStore();
    const stream = eventStore.getStream(this.activeSlot.streamId);
    if (!stream) return;
    
    // Mark all events in stream as frozen (user-owned)
    // In a real implementation, we'd track which events were generated
    // and only freeze those. For now, we just clear the generated flag.
    ...
    const originalPayloads = new Map(
      events.map(event => [event.id, { payload: event.payload }])
    );
    
    getUndoStack().push({
      type: 'batch',
      description: 'Freeze generated events',
      redo: () => {
        eventStore.updateEvents(this.activeSlot!.streamId, updates);
      },
      undo: () => {
        eventStore.updateEvents(this.activeSlot!.streamId, originalPayloads);
      }
    });
    
    console.log('Froze events in stream', this.activeSlot.streamId);
  }
  
  /**
   * Humanize action (G078: add timing/velocity variation)
   */
  humanize(amount: number = 0.2): void {
    if (!this.activeSlot) return;
    ...
    for (const event of originalEvents) {
      ...
      originalUpdates.set(event.id, {
        start: event.start,
        payload: event.payload
      });
      
      // Add timing variation
      const timingOffset = (Math.random() - 0.5) * amount * 30;
      const newStart = asTick(Math.max(0, event.start + Math.floor(timingOffset)));
      
      // Add velocity variation
      const velocityOffset = (Math.random() - 0.5) * amount * 40;
      const currentVelocity = (event.payload as any).velocity || 80;
      const newVelocity = Math.max(40, Math.min(127, Math.floor(currentVelocity + velocityOffset)));
    ...
    getUndoStack().push({
      ...
      undo: () => {
        eventStore.updateEvents(this.activeSlot!.streamId, originalUpdates);
      }
    });
    
    console.log('Humanized events in stream', this.activeSlot.streamId);
  }
  
  /**
   * Quantize action (G078: snap to grid)
   */
  quantize(gridSize: number = 120): void {
    if (!this.activeSlot) return;
    ...
    for (const event of originalEvents) {
      // Snap start to nearest grid position
      const newStart = asTick(Math.round(event.start / gridSize) * gridSize);
      
      // Only update if changed
      if (newStart !== event.start) {
        originalUpdates.set(event.id, { start: event.start });
        updates.set(event.id, { start: newStart });
      }
    }
    
    if (updates.size === 0) {
      console.log('No events needed quantization');
      return;
    }
    ...
    console.log('Quantized events in stream', this.activeSlot.streamId);
  }
...
function injectGeneratorDeckStyles(): void {
  ...
  style.textContent = `
    ...
    .generator-deck__actions {
      padding-top: var(--spacing-lg, 24px);
      border-top: 1px solid var(--color-border, #444);
    }
    
    .generator-deck__actions h4 {
      margin: 0 0 var(--spacing-md, 16px) 0;
      font-size: 14px;
      font-weight: 600;
      color: var(--color-text-primary, #fff);
    ...
    .generator-deck__actions-row {
      display: flex;
      gap: var(--spacing-sm, 8px);
    }
    
    .generator-deck__action-btn {
      flex: 1;
    ...
    .generator-deck__action-btn:hover {
      background: var(--color-bg-hover, rgba(255, 255, 255, 0.1));
      border-color: var(--color-primary, #3b82f6);
    }
    
    .generator-deck__action-btn:active {
